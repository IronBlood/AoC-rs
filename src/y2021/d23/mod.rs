...
impl<const R: usize> State<R> {
    ...
    fn is_above_room(&self, x: usize) -> bool {
        x >= 2 && x % 2 == 0 && (x - 2) / 2 < self.rooms.len()
    }
...
fn input_generator(input: &str) -> Vec<Amphipod> {
    let amphipods = input
        .chars()
...
pub fn run(input: &str) {
    let amphipods = input_generator(input);
    ...
    println!("part 1: {}", solve_both_parts(initial_state));
    ...
    println!("part 2: {}", solve_both_parts(initial_state));
}
